...
```
Деплой проекта
Проект использует GitHub Actions для автоматизации CI/CD-процесса. Весь процесс включает следующие этапы:

Проверка кода (Linting):

Код проверяется с помощью flake8, чтобы убедиться, что он соответствует стандартам оформления.

Запуск тестов:

Запускаются автоматические тесты с использованием Django. Для тестирования используется база данных PostgreSQL.

Сборка Docker-образа:

Из исходного кода собирается Docker-образ.

Загрузка образа в Docker Hub:

Собранный Docker-образ загружается в Docker Hub.

Деплой на сервер:

Приложение разворачивается на целевом сервере с использованием SSH.
